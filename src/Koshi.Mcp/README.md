...
| `KOSHI_MEMORY_VAULT` | _(unset — vault stays opt-in)_ | Path to a directory used to persist memories as **one Markdown file per memory** under `<vault>/koshi/<type>/`. Git-friendly, Obsidian-compatible. External edits/deletes are picked up automatically via a file-system watcher (see `KOSHI_VAULT_WATCH`). Relative paths resolve against `KOSHI_PROJECT_ROOT`. See [docs/vault-mode.md](../../docs/vault-mode.md) for the full format spec and migration guide. |
