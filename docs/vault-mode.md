...
|---|---|
| **0.6.0** (current) | Vault backend, three sync tools, unmanaged-note reporting, per-call disk reload. |
| 0.6.1 | `FileSystemWatcher`-backed cache invalidation — skip the per-call scan when the cache is known fresh. |
| 0.7.0 | Multi-flavor file layouts: Logseq (`pages/*.md`), Dendron (`koshi.<type>.<slug>.md`). The frontmatter schema is unchanged across flavors. |
